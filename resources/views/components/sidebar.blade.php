<aside x-data="{ open: true, hasNewNotification: false }" {{-- CAMBIADO: 'open: true' por defecto --}}
    class="fixed top-4 left-4 h-[calc(100vh-2rem)] transition-all duration-300 bg-gray-900 rounded-xl shadow-lg flex flex-col items-center py-4 z-50"
    :class="open ? 'w-64 items-start' : 'w-20 items-center'"> {{-- ELIMINADOS @mouseenter y @mouseleave --}}
    ...
    <nav class="flex-1 w-full space-y-2 px-2 text-gray-300"> 
        ...
        @canany(['cliente-access', 'entrenador-access', 'admin_entrenador'])
            ...
            <x-sidebar-link icon="message-circle" route="chat.index" label="Mensajes" />
        @endcan
    </nav>
