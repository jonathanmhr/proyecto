    ...
    <div class="container mx-auto p-6 lg:p-12 bg-gray-900 min-h-screen text-white font-sans">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-10">
            <h1 class="text-4xl font-extrabold text-green-400 text-center sm:text-left leading-tight mb-4 sm:mb-0">
                Planificar tu Entrenamiento: <span
                    class="block sm:inline-block mt-2 sm:mt-0 text-white">{{ $entrenamiento->titulo }}</span>
            </h1>
        ...
        @if ($entrenamiento->fases->isEmpty())
            <div class="bg-gray-800 p-10 rounded-2xl shadow-xl text-center border border-gray-700 animate-fade-in">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 text-gray-500 mx-auto mb-8 animate-bounce-slow"
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-12">
                ...
                <div
                    class="bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-700 transition-all duration-300 transform hover:scale-[1.01] hover:shadow-2xl animate-slide-in-left">
                    <h3
                        class="text-3xl font-extrabold text-green-400 mb-6 flex items-center gap-3 border-b border-gray-700 pb-4">
                    ...
                    <form method="POST" action="{{ route('cliente.entrenamientos.fases-dias.store', $entrenamiento) }}"
                        class="space-y-6">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha" id="fecha" required
                                min="{{ $fechaMin->format('Y-m-d') }}" max="{{ $fechaMax->format('Y-m-d') }}"
                                {{-- ¡Aquí se usan las variables del controlador! --}}
                ...
                <div class="bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-700 animate-slide-in-right">
                    ...
                    @if ($entrenamiento->diasPlanificados->isEmpty())
                        <div class="text-center py-10">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-gray-600 mx-auto mb-6"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h10a2 2 0 012 2v4a2 2 0 002 2h4a2 2 0 002-2V7a2 2 0 00-2-2h-2.586a1 1 0 01-.707-.293l-1.121-1.121A1 1 0 009.121 3H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-4a2 2 0 00-2-2h-4a2 2 0 00-2 2v4a2 2 0 002 2z" />
                            </svg>
                            <p class="text-gray-400 text-lg">Aún no has planificado ninguna fase para este
                                entrenamiento.</p>
                    ...
                    @else
                        <ul class="space-y-4">
                            @foreach ($entrenamiento->diasPlanificados->sortBy('fecha') as $faseDia)
                                <li
                                    class="bg-gray-700 p-4 rounded-lg shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between border border-gray-600 transition-all duration-200 transform hover:scale-[1.005]">
                                    <div class="flex items-center gap-4 mb-3 sm:mb-0">
                                        <div
                                            class="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                                            {{ \Carbon\Carbon::parse($faseDia->fecha)->format('d') }}
                                        </div>
                                        <div>
                                            <span
                                                class="font-semibold text-xl text-white block">{{ $faseDia->fase->nombre }}</span>
                                            <p class="text-gray-400 text-sm">Fecha: <span
                                                    class="font-medium text-white">{{ \Carbon\Carbon::parse($faseDia->fecha)->locale('es')->isoFormat('dddd, DD MMMM YYYY') }}</span>
                                            </p> {{-- Asegurando que el año también se muestre --}}
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-4 w-full sm:w-auto justify-end sm:justify-start">
                                        <span
                                            class="px-4 py-1 rounded-full text-sm font-semibold {{ $faseDia->estado === 'completado' ? 'bg-green-500 text-white' : ($faseDia->estado === 'pendiente' ? 'bg-yellow-500 text-black' : 'bg-gray-500 text-white') }} flex-shrink-0">
                                            {{ ucfirst($faseDia->estado) }}
                                        </span>
<div class="flex gap-2 flex-shrink-0">
    <form
        action="{{ route('cliente.entrenamientos.fases-dias.updateEstado', $faseDia->id) }}"
        method="POST"
        class="form-estado"
    >
        @csrf
        ...
        <button
            type="submit"
            class="btn-estado text-green-400 hover:text-green-500 transition-colors duration-200 p-2 rounded-full hover:bg-gray-600"
            title="{{ $faseDia->estado === 'completado' ? 'Marcar como pendiente' : 'Marcar como completado' }}"
            data-estado="{{ $faseDia->estado }}"
        >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="{{ $faseDia->estado === 'completado' ? 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z' : 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
            </svg>
        </button>
    </form>

    <!-- El formulario de eliminar queda igual -->
    <form
        action="{{ route('cliente.entrenamientos.fases-dias.destroy', $faseDia->id) }}"
        method="POST"
        onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta fase planificada?');"
    >
        @csrf
        @method('DELETE')
        <button type="submit"
            class="text-red-400 hover:text-red-500 transition-colors duration-200 p-2 rounded-full hover:bg-gray-600"
            title="Eliminar fase"
        >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
document.querySelectorAll('.form-estado').forEach(form => {
  form.addEventListener('submit', function(event) {
    ...
    if (!confirm('¿Quieres cambiar el estado de esta fase?')) {
      return;
    }

    const boton = form.querySelector('button.btn-estado');
    const estadoActual = boton.dataset.estado; // 'completado' o 'pendiente'
    ...
    .then(data => {
      ...
      // Actualiza data-estado y título del botón
      boton.dataset.estado = data.dia.estado;

      if (data.dia.estado === 'completado') {
        boton.title = 'Marcar como pendiente';
        boton.querySelector('path').setAttribute('d', 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z');
      } else {
        boton.title = 'Marcar como completado';
        boton.querySelector('path').setAttribute('d', 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z');
      }
    })
    .catch(error => {
