<x-app-layout>
    <div class="container mx-auto px-6 py-10 sm:px-8 lg:px-12 bg-gray-900 min-h-screen text-gray-200">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-12 animate-fade-in-down">
            <h1 class="text-4xl lg:text-5xl font-extrabold text-green-400 text-center sm:text-left leading-tight mb-4 sm:mb-0">
                Mis Entrenamientos Guardados <span class="block text-gray-400 text-xl font-medium mt-2">Gestiona tus rutinas de fitness personalizadas</span>
            </h1>
        ...
        @if($entrenamientos->isEmpty())
            ...
            <div class="bg-gray-800 p-10 rounded-2xl shadow-xl text-center border border-gray-700 animate-fade-in">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 text-gray-600 mx-auto mb-8 animate-bounce-slow" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                ...
                <p class="text-gray-400 text-lg mb-8 max-w-2xl mx-auto">
                    Explora nuestra biblioteca para encontrar la rutina perfecta para ti y empieza a planificar tu progreso.
                </p>
                ...
                @foreach($entrenamientos as $index => $entrenamiento)
                    <div class="bg-gray-800 rounded-2xl shadow-xl border border-gray-700 p-6 flex flex-col justify-between
                        ...
                        <div>
                            ...
                            <h2 class="text-3xl font-bold text-green-400 mb-3 leading-snug">{{ $entrenamiento->titulo }}</h2>
                            <p class="text-gray-300 text-base leading-relaxed mb-4 line-clamp-3">{{ $entrenamiento->descripcion }}</p>
                            ...
                            <div class="mb-5 text-gray-400 text-lg font-medium space-y-2">
                                ...
                                <p class="flex items-center gap-3">
                                    ...
                                    Días: <span class="font-semibold text-white">{{ $entrenamiento->duracion_dias ?? 'N/A' }}</span>
                                </p>
                            </div>
                            ...
                            @if($entrenamiento->fases->isNotEmpty())
                                <div class="mb-4">
                                    <h3 class="text-xl font-bold text-white mb-3 border-b border-gray-700 pb-2 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16" />
                                        </svg>
                                        Fases:
                                    </h3>
                                    <ul class="space-y-3">
                                        @foreach($entrenamiento->fases->take(2) as $fase) {{-- Mostrar solo las primeras 2 fases --}}
                                            <li class="bg-gray-700 p-3 rounded-lg shadow-inner border border-gray-600">
                                                <h4 class="text-lg font-bold text-green-300 mb-1 flex items-center gap-2">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                                                    </svg>
                                                    {{ $fase->nombre }}
                                                </h4>
                                                @if($fase->actividades->isNotEmpty())
                                                    <ul class="list-disc list-inside text-gray-400 text-sm mt-2 ml-4 space-y-1">
                                                        @foreach($fase->actividades->take(2) as $actividad) {{-- Mostrar solo 2 actividades por fase --}}
                                                            <li>
                                                                <span class="font-medium text-white">{{ $actividad->nombre }}</span>
                                                                @if($actividad->series) - Series: {{ $actividad->series }} @endif
                                                                @if($actividad->repeticiones) - Reps: {{ $actividad->repeticiones }} @endif
                                                            </li>
                                                        @endforeach
                                                        @if($fase->actividades->count() > 2)
                                                            <li class="italic text-gray-500">...y {{ $fase->actividades->count() - 2 }} actividades más.</li>
                                                        @endif
                                                    </ul>
                                                @else
                                        ...
                                        @if($entrenamiento->fases->count() > 2)
                                            <p class="text-gray-500 italic mt-2 text-sm text-center">Ver todas las fases en el detalle (requiere otra página/ruta).</p>
                                        @endif
                                    </ul>
                                </div>
                            @else
                                <p class="text-gray-500 italic mb-4">Este entrenamiento no tiene fases definidas aún.</p>
                            @endif
                        </div>
                        ...
                        <div class="mt-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 sm:space-x-4">
                            ...
                            <form action="{{ route('cliente.entrenamientos.quitar', $entrenamiento->id) }}" method="POST"
                                  class="w-full sm:w-auto" onsubmit="return confirm('¿Estás seguro de que quieres quitar este entrenamiento de tus guardados? Esta acción no se puede deshacer.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
    ...
    @push('styles')
        <style>
            ...
            @keyframes fadeInRight {
                from { opacity: 0; transform: translateX(-40px); } /* Empieza más a la izquierda */
                to { opacity: 1; transform: translateX(0); }
            }
            .animate-fade-in-right {
                animation: fadeInRight 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards; /* Curva de aceleración */
            }
