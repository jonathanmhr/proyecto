            ...
            <div class="bg-white shadow-md rounded-lg p-6 space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <select name="clases[]" id="clases"
                        class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" multiple>
                        @foreach ($clases as $clase)
                            <option value="{{ $clase->id }}" @if ($entrenador->clasesGrupales->contains($clase->id)) selected @endif
                                {{ $clase->nombre }} </option>
                        @endforeach
                    </select>
