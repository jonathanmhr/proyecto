                ...
                <form method="POST" action="{{ route('admin-entrenador.entrenamientos.update', $entrenamiento->id_entrenamiento) }}">
                    ...
                    <div class="space-y-6">
                        <div class="mb-4">
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $entrenamiento->nombre) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="tipo" id="tipo" value="{{ old('tipo', $entrenamiento->tipo) }}"
                                class="w-full p-3 bg-gray-700 text-white border border-gray-600 rounded-lg focus:outline-none focus:border-red-500 focus:ring-red-500 transition duration-200" required>
                        </div>

                        <div class="mb-4">
                            <label for="duracion" class="block text-white font-semibold mb-2">Duración (min)</label>
                            <input type="number" name="duracion" id="duracion" value="{{ old('duracion', $entrenamiento->duracion) }}"
                                class="w-full p-3 bg-gray-700 text-white border border-gray-600 rounded-lg focus:outline-none focus:border-red-500 focus:ring-red-500 transition duration-200" required>
                        </div>

                        <div class="mb-4">
                            <label for="fecha" class="block text-white font-semibold mb-2">Fecha</label>
                            <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $entrenamiento->fecha) }}"
                                min="{{ now()->format('Y-m-d') }}" max="{{ now()->addMonths(6)->format('Y-m-d') }}"
                                class="w-full p-3 bg-gray-700 text-white border border-gray-600 rounded-lg focus:outline-none focus:border-red-500 focus:ring-red-500 transition duration-200" required>
                        </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                // Apply min/max attributes if not already set by Blade
