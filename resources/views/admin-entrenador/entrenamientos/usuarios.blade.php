<x-app-layout>
    {{-- El header slot se mantiene vacío si no se necesita contenido adicional global --}}
        ...
        <div class="max-w-xl md:max-w-4xl lg:max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-800 rounded-xl shadow-xl border border-gray-700 p-6 md:p-8">
                @if($entrenamiento->usuariosGuardaron->isEmpty())
                    {{-- Mensaje para cuando no hay usuarios guardados --}}
                ...
                @else
                    {{-- Título de la sección de lista de usuarios --}}
                    <h2 class="text-2xl font-bold text-white mb-6 border-b border-gray-700 pb-3 flex items-center">
                        {{-- Icono de usuarios de Feather Icons --}}
                        <i data-feather="users" class="w-6 h-6 mr-3 text-red-500"></i> Lista de Usuarios Guardados
                    </h2>

                    {{-- Contenedor de la lista de usuarios con espaciado entre elementos --}}
                    <div class="space-y-4">
                        @foreach($entrenamiento->usuariosGuardaron as $usuario)
                            ...
                            <div class="bg-gray-700 p-5 rounded-lg shadow-md border border-gray-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                <div class="flex items-center">
                                    {{-- Icono de usuario de Feather Icons --}}
                                ...
                                {{-- Aquí podrías añadir más información del usuario o acciones si fueran necesarias --}}
                            </div>
                        @endforeach
                    </div>
                @endif
