<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-xl text-white leading-tight">Crear nuevo rol</h2>
    </x-slot>

    <div class="py-6 max-w-lg mx-auto sm:px-6 lg:px-8">
        {{-- Formulario - Fondo oscuro, padding, esquinas redondeadas, sombra y borde --}}
        <form action="{{ route('admin.roles.store') }}" method="POST" class="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
            @csrf
            ...
            <div class="flex justify-between items-center mt-6"> {{-- Margen superior para separación --}}
                ...
                {{-- Botón Guardar - Azul para acción principal --}}
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 shadow transition-colors">Guardar</button>
            </div>
        </form>
    </div>
</x-app-layout>